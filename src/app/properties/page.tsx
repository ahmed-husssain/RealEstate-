...
function PropertiesContent() {
  ...
  const filteredProperties = useMemo(() => {
    return mockProperties.filter((property) => {
      ...
      // Neighborhood
      if (filters.neighborhood !== 'all' && property.location.neighborhood !== filters.neighborhood) {
        return false;
      }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2 border-b border-[#d8cebe]/60 pb-4">
        <div className="flex items-center gap-2 text-xs font-mono text-[#7e7365]">
          <span>Showing <strong className="text-[#1F1B16]">{filteredProperties.length}</strong> of {mockProperties.length} residences</span>
        </div>
...
export default function PropertiesPage() {
  return (
    <Suspense fallback={<div className="max-w-7xl mx-auto p-12 text-center font-mono text-xs text-[#7e7365]">Loading catalog...</div>}>
      <PropertiesContent />
    </Suspense>
  );
}
